...
volatile uint8_t Rcp_Outside_Range_Cnt; // RC脉冲超出范围计数
volatile uint8_t Rcp_Stop_Cnt;      // RC脉冲停止计数
volatile uint8_t Rcp_Timeout_Cntd;  // RC脉冲超时计数
...
volatile uint8_t Initial_Run_Cnt;    // 初始运行计数
volatile uint8_t Startup_Timeout_Cntd; // 启动超时计数
...
volatile uint16_t Dshot_Frame_Cnt;
volatile uint16_t Last_Dshot_Frame_Cnt;
...
#define DSHOT_CMD_THRESHOLD 12      // DShot命令阈值
volatile uint8_t Beep_Cmd;          // 蜂鸣命令
...
volatile uint16_t Pgm_Throttle_Cal_Min;  // 编程油门校准最小值
volatile uint16_t Pgm_Throttle_Cal_Max;  // 编程油门校准最大值
volatile uint16_t Pgm_Throttle_Cal_Range; // 编程油门校准范围
volatile uint8_t Pgm_Throttle_Cal;       // 编程油门校准标志
volatile uint8_t Throttle_Cal_Timeout_Cnt; // 油门校准超时计数
...
/**
 * 无信号初始化
 * 检测输入信号类型，包括常规PWM、OneShot125、OneShot42、DShot和Multishot
 */
void init_no_signal(void) {
    // 禁用所有中断
    SFR.IE_EA = 0;
    ...
    // 初始化RC脉冲
    Flags2.RCP_UPDATED = 0;  // 清除更新标志
    wait200ms();
    
    // 清除所有信号类型标志
    Flags2.RCP_ONESHOT125 = 0;  // 清除OneShot125标志
    Flags2.RCP_ONESHOT42 = 0;   // 清除OneShot42标志
    Flags2.RCP_MULTISHOT = 0;   // 清除Multishot标志
    Flags2.RCP_DSHOT = 0;       // 清除DShot标志
    Dshot_Cmd = 0;              // 清除DShot命令
    Dshot_Cmd_Cnt = 0;          // 清除DShot命令计数
    
    // 测试信号是否为常规PWM
    Rcp_Outside_Range_Cnt = 0;  // 重置超出范围计数器
    wait100ms();                // 等待新的RC脉冲
    if (Rcp_Outside_Range_Cnt < 10) {
        // 信号是常规PWM，验证并开始
        return;
    ...
    // 测试信号是否为OneShot125
    Flags2.RCP_ONESHOT125 = 1;  // 设置OneShot125标志
    Rcp_Outside_Range_Cnt = 0;  // 重置超出范围计数器
    wait100ms();                // 等待新的RC脉冲
    if (Rcp_Outside_Range_Cnt < 10) {
        // 信号是OneShot125，验证并开始
    ...
    Flags2.RCP_ONESHOT42 = 1;   // 设置OneShot42标志
    Rcp_Outside_Range_Cnt = 0;  // 重置超出范围计数器
    wait100ms();                // 等待新的RC脉冲
    if (Rcp_Outside_Range_Cnt < 10) {
        // 信号是OneShot42，验证并开始
    ...
    // 为DShot150设置变量
#if MCU_48MHZ >= 1
    DShot_Timer_Preset = 128;  // 加载DShot同步定时器预设值（用于DShot150）
#else
    DShot_Timer_Preset = 192;
#endif
    DShot_Pwm_Thr = 20;         // 加载DShot资格PWM阈值（用于DShot150）
    DShot_Frame_Length_Thr = 80; // 加载DShot帧长度标准
    
    // 测试信号是否为DShot150
    Flags2.RCP_ONESHOT42 = 0;
    Flags2.RCP_DSHOT = 1;        // 设置DShot标志
    Rcp_Outside_Range_Cnt = 10;  // 设置超出范围计数器
    wait100ms();                 // 等待新的RC脉冲
    DShot_Pwm_Thr = 16;          // 加载DShot常规PWM阈值
    if (Rcp_Outside_Range_Cnt < 10) {
        // 信号是DShot150
        Dshot_Cmd = 0;
        Dshot_Cmd_Cnt = 0;
    ...
    // 为DShot300设置变量
    // 设置定时器0/1时钟为系统时钟（用于DShot300）
#if MCU_48MHZ >= 1
    DShot_Timer_Preset = 0;     // 加载DShot同步定时器预设值（用于DShot300）
#else
    DShot_Timer_Preset = 128;
#endif
    DShot_Pwm_Thr = 40;          // 加载DShot资格PWM阈值（用于DShot300）
    DShot_Frame_Length_Thr = 40;  // 加载DShot帧长度标准
    
    // 测试信号是否为DShot300
    Rcp_Outside_Range_Cnt = 10;   // 设置超出范围计数器
    wait100ms();                  // 等待新的RC脉冲
    DShot_Pwm_Thr = 32;           // 加载DShot常规PWM阈值
    if (Rcp_Outside_Range_Cnt < 10) {
        // 信号是DShot300
        Dshot_Cmd = 0;
        Dshot_Cmd_Cnt = 0;
    ...
    // 为DShot600设置变量
    // 设置定时器0/1时钟为系统时钟（用于DShot600）
#if MCU_48MHZ >= 1
    DShot_Timer_Preset = 128;    // 加载DShot同步定时器预设值（用于DShot600）
#else
    DShot_Timer_Preset = 192;
#endif
    DShot_Pwm_Thr = 20;           // 加载DShot资格PWM阈值（用于DShot600）
    DShot_Frame_Length_Thr = 20;   // 加载DShot帧长度标准
